...
# 前言
此教程在熱心的客戶幫助下，一直在迭代更新，目前您看到的是第七版（2022.1.31）。

我強烈建議您看完教程，一來這是很多玩家智慧的結晶；二來我一直認為折騰硬件和談戀愛是一個道理，其中的過程是最美妙，成功駕馭後，獲得的滿足感是無可替代的。

教程的篇幅看起來比較長，但是請不要膽怯，我只是在玩家們的幫助下將50個字延展到了這麼長，為的是讓每一個過來的玩家都能看懂並且可操作，所以事無巨細稍顯囉嗦，在您之前，已經有1300+人（2021.12.20）成功讓群暉安裝了2.5G網卡（數據源自店內銷售統計，因為教程是開放的，實際人數應該大於這個數字），仔細的對照操作，成功是確定性事件。

![20210908092118](https://user-images.githubusercontent.com/85718974/132430593-1a942122-6613-4f24-b966-6a55e9728f02.jpg)

**發問前請先看完教程**

1.教程**支持白群暉、黑群暉**；支持大部分ARM處理器的群暉；

2.開始教程前，請先確定您的DSM系統的版本，可以支持的是DSM6.2.x和DSM7.x.x，目前不支持DSM6.1及更早系統！如果您的系統版本不支持，請升級，升級不會破壞數據；

3.有些早期ARM型號比如DS218Play，暫時沒有支持DSM7.0的驅動，請降級，降級亦不會破壞數據。
...
```
問你yes不yes，當然要yes了！
yes之後回車輸入群暉登錄密碼(輸入的時候沒有光標沒有提示什麼都沒有不要以為是鍵盤壞了哦)
...
## 還沒完呢？別急吼吼的關閉教程去重啟。
如果您的機器是DS3615xs、DS3617xs等含有4個或4個以上的網口的型號，請跳過第四大點直接看第五大點；

如果您的機器是DS918+、218j等只有1到2個網口的型號，請繼續往下執行教程，一條都不要落下。
...
和文件1的操作一樣，找到maxlanport=「2」這個字段，按「i」進入編輯模式，修改成和前面一樣的數量（在81%位置左右）。
...
如果出現了這個畫面，請選擇「E」，這樣就進入了編輯模式。**注意，上圖畫面不一定會出現，如果沒有出現，請直接進入下面的流程，不用按「E」進入編輯模式。**

按方向鍵↓滾屏，找到有個含有「sleep 10」的語句，按「i」進入編輯模式，將其修改成「sleep 30」。

修改這個數字的意義是告訴系統啟動之後延遲30秒啟動網卡驅動，如果您的機器配置嗷嗷叫，可以適當把時間改短一點，白群暉老老實實改成30吧。
...
## 2.重啟還是沒有，但是關了再開一次套件就有了？
問題出在有個腳本的等待時間不夠，我們要SSH登錄上去修改一個參數，請看第五大點。
## 3.還是不行？您的機型是DS2xx+、DS4xx、DS9xx ？
有些機型的默認配置參數只能支持1-2個網卡，增加的網卡當然識別不了，需要修改網卡數量的參數，請看第四大點。
## 4.仍有問題？
那麼屬於疑難雜症了，請參閱：[https://fu.chiphello.com/2021/06/a-knotty-problem-2.5g/](https://fu.chiphello.com/2021/06/a-knotty-problem-2.5g/)
